...
@section('panel-body')
    ...
    <h3 class="mt-4" id="sleep-exceptions">Sleep exceptions</h3>
    <p class="text-muted">
        Date ranges (inclusive, in your configured timezone) during which no automatic sleep time is generated by
        the availability API — useful when travelling and your sleep schedule is unknown. Calendar events matching
        your configured nap event name still count as sleep during these dates.
    </p>

    @if($sleepExceptions->isEmpty())
        <p class="text-muted small">No sleep exceptions yet.</p>
    @else
        <table class="table table-sm align-middle w-auto mb-3">
            <thead>
                <tr>
                    <th>From</th>
                    <th>To</th>
            ...
            <tbody>
                @foreach($sleepExceptions as $exception)
                    <tr>
                        <td class="pe-3">{{ $exception->start_date->format('Y-m-d') }}</td>
                        <td class="pe-3">{{ $exception->end_date->format('Y-m-d') }}</td>
                        <td class="pe-3">{{ $exception->label ?? '—' }}</td>
                        <td>
                            <form method="POST" action="/dashboard/sleep-exceptions/{{ $exception->id }}">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="btn btn-sm btn-outline-danger"
                                        onclick="return confirm('Remove this sleep exception?')">Remove</button>
                            </form>
                        </td>
                    </tr>
                @endforeach
    ...
    <form method="POST" action="/dashboard/sleep-exceptions" class="d-flex gap-2 align-items-end flex-wrap mb-4">
        @csrf
        <div>
            <label for="exception-start" class="form-label mb-1 small">From</label>
            <input type="date"
                   class="form-control form-control-sm @error('start_date') is-invalid @enderror"
                   id="exception-start" name="start_date" value="{{ old('start_date') }}" required>
            @error('start_date')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
        <div>
            <label for="exception-end" class="form-label mb-1 small">To</label>
            <input type="date"
                   class="form-control form-control-sm @error('end_date') is-invalid @enderror"
                   id="exception-end" name="end_date" value="{{ old('end_date') }}" required>
            @error('end_date')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
        <div>
            <label for="exception-label" class="form-label mb-1 small">Label (optional)</label>
            <input type="text" class="form-control form-control-sm" id="exception-label" name="label"
                   maxlength="255" placeholder="e.g. Travel" value="{{ old('label') }}" style="width:12rem">
        </div>
        <button type="submit" class="btn btn-sm btn-outline-primary">Add exception</button>
    </form>
