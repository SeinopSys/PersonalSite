...
class AvailabilityResult
{
     ...
     * @param TimeSlot[] $highlighted Time slots of calendar events with a "with <token>"/"w/ <token>" clause matching one of the token's words. These events still count as busy. Tentative events (name ending in "(?)") include a `tentative: true` field. Each slot includes an `activity` field: the freetext preceding "with"/"w/", trimmed of trailing whitespace.
     ...
     * @param TimeSlot[] $sleep       Time slots outside the configured wake/sleep window (and any day marked fully unavailable), plus any calendar event matching the configured nap event name, sorted chronologically. Directly adjacent or overlapping blocks are merged into one.
     */
