...
class DashboardController extends Controller
{
    ...
    public function availability(Request $request)
    {
        ...
        $data = [
            ...
            'sleepExceptions'    => $user->sleepExceptions()->orderBy('start_date')->get(),
        ];
    ...
    /** Adds a "sleep exception" date range during which no automatic sleep block is generated by the API. */
    public function storeSleepException(Request $request)
    {
        $validated = $request->validate([
        ...
        Auth::user()->sleepExceptions()->create([
            'start_date' => $validated['start_date'],
            'end_date'   => $validated['end_date'],
            'label'      => ($validated['label'] ?? '') !== '' ? $validated['label'] : null,
        ]);

        return redirect('/availability#sleep-exceptions')->with('success', 'Sleep exception added.');
    }

    public function destroySleepException(string $id)
    {
        SleepException::where('id', $id)
            ->where('user_id', Auth::id())
            ->firstOrFail()
            ->delete();

        return redirect('/availability#sleep-exceptions')->with('success', 'Sleep exception removed.');
    }
